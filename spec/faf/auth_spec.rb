...
describe Faf::Auth do
  context 'username' do
    context 'without a username configured' do
      before do
        expect(Faf::Shell).to receive(:system!).with('git config github.user').and_return(nil)
      end
    ...
    context 'with a username configured' do
      it 'returns username' do
        expect(Faf::Shell).to receive(:system!).with('git config github.user').and_return('user')
    ...
    context 'with git not configured' do
      before do
        expect(Faf::Shell).to receive(:system!).with('git config github.user').and_raise(RuntimeError)
      end
